    ...
    return (
        <div className="rel flex {flex-wrap:wrap}@<sm bg:gray-10 bg:gray-90@light r:3">
            <div className="rel flex flex:1|0|100% flex:1@sm aspect-ratio:16/9 overflow:clip r:3">
                <Image src={party.image ?? defaultImage} fill sizes="100%" className="object-fit:cover user-drag:none" alt="preview" loading="eager" />
            </div>
            {
                user?.uid === party.createdBy &&
                <Link href={`/${locale}/${party.id}/edit`} onClick={() => nProgress.start()}>
                    <button className="abs top:16 right:16 r:3 p:4 bg:gray-10 bg:gray-90@light
            ...
            <div className="flex flex:1 w:0 flex:col mx:16 ml:32@md justify-content:center">
                <h1 className="f:28 f:24@<md mt:20 white-space:nowrap overflow:clip text-overflow:ellipsis user-select:text">
                    {party.name}
                </h1>
                <h2 className="f:18 {f:16;my:4}@<md my:8 font-weight:normal fg:gray-60 fg:gray-50@light user-select:text">{party.startDate} ~ {party.endDate}</h2>
                <p className="mt:16 mb:42 f:18 f:16@<md user-select:text">
                    {party.desc}
